...
global errors, or how does the numerical method behave when \(t\to\infty\). As in the
previous case we will show an example for the leapfrog method, following the
ideas exposed in~\cite{butcher_numericalmethods_2016}
and in~\cite{leimkuhler_simulatinghamiltonian_2004}.

In the following we will only consider linear stability analysis and thus we
will apply the leapfrog method to a linear problem.

Let us denote the dynamical state of the system with
\[
\vb{z} =
\begin{pmatrix}
  \vb{q} \\
...
it to obtain concrete results regarding stability. Let us consider the Hamiltonian
\[
\mathcal{H} = \frac{\vb{p}^2}{2m} + \frac{\omega^2 \vb{q}^2}{2}
\]
and the associated Hamilton equations
\begin{align*}
  \dot{\vb{q}} &= \pdv{\mathcal{H}}{p} = \frac{p}{m} \\
  \dot{\vb{p}} &= -\pdv{\mathcal{H}}{q} = -\omega^2 q\,.
\end{align*}
...
The discrete version of the linear problem is
\begin{equation}
...
Moreover, we can iteratively construct the solution at the \(n\)-th
timestep from the initial conditions
\[
\vb{z}_n = \left[\hat{R}\right]^n \vb{z}_0
\]
and if we analyze the behavior of the eigenvalues of \(\hat{R}\),
we can get an insight for the long time behavior.
