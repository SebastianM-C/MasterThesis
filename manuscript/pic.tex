...
\begin{definition}
  ...
  \emph{symplectic} if its corresponding Jacobian matrix \(g'(\vb{p},\vb{q})\) is evereywhere symplectic, i.e.
  \[
...
\begin{definition}
  ...
  mainfold and for which the product is given by the differentiable
  mapping \(G \cp G \to G\).
\end{definition}
...
\end{pmatrix}\,,
\]
the simplecticty condition can be written as
\begin{align*}
  \begin{pmatrix}
    S_1^T & S_3^T \\
    S_2^T & S_4^T
...
If we consider the simplifed case of homogenuous electric and magnetic fields,
then
\[
...
\Textcite{kaneyee_numericalsolution_1966} propoed a method for solving Maxwell's
